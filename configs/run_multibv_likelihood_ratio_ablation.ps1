# Run from the repository root. Each arm uses the same 14 validation templates,
# ten repeats, simulator seeds, Seed19 policy, and intervention budget.
...
foreach ($limit in $ablationLimits) {
    $ablationOutput = "data_analysis\raw_data\shrp2_closed_loop_validation14_x10_audited5_seed19_lr${limit}_budget10_v1"
...
foreach ($limit in $ablationLimits) {
    $ablationOutput = "data_analysis\raw_data\shrp2_closed_loop_validation14_x10_audited5_seed19_lr${limit}_budget10_v1"
    $ablationLog = "data_analysis\logs\shrp2_closed_loop_validation14_x10_audited5_seed19_lr${limit}_budget10_v1.log"
    $arguments = @(
