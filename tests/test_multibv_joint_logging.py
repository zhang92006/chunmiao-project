...
class MultiBVJointLoggingTests(unittest.TestCase):
    ...
    def test_joint_context_contains_k_agents_and_probability_products(self):
        ...
        self.assertTrue(np.allclose(
            controller.control_log["weight_record"][
    ...
    def test_joint_log_aligns_named_epsilon_with_recorded_vehicle_order(self):
        controller = NADEBVGlobalController.__new__(NADEBVGlobalController)
        controller.joint_control_num = 2
        controller.control_log = {
            "epsilon_by_bv_id": {"BV_context": 0.9, "BV_primary": 0.1}
        }
        controller.drl_epsilon_value = [0.9, 0.1]
        controller.real_epsilon_value = [0.9, 0.1]
        controller.env = SimpleNamespace(
            initial_weight=1.0,
            info_extractor=SimpleNamespace(episode_log={"weight_episode": 1.0}),
        )
        ...
        controller._record_joint_training_context(
            bvs, [0.2, 0.3], [0.01, 0.02], [0.5, 0.4]
        )

        self.assertEqual(controller.drl_epsilon_value, [0.1, 0.9])
        self.assertEqual(controller.real_epsilon_value, [0.1, 0.9])
