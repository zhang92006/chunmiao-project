...
class Simulator(object):    
    ...
    def __init__(self,
        ...
        if gui_flag:
            self.sumo_binary = self._resolve_sumo_binary("sumo-gui")
        else:
            self.sumo_binary = self._resolve_sumo_binary("sumo")
        ...
        self.gui_delay = int(conf.simulation_config.get("gui_delay", 100))
        self.gui_start = bool(conf.simulation_config.get("gui_start", True))

    @staticmethod
    def _resolve_sumo_binary(name):
        sumo_home = os.environ.get("SUMO_HOME")
        executable = f"{name}.exe" if os.name == "nt" else name
        if sumo_home:
            candidate = os.path.join(sumo_home, "bin", executable)
    ...
    def track_vehicle_gui(self, vehID="CAV"):
        ...
        try:
            ...
            traci.gui.setSchema(viewID='View #0', schemeName='real world')
            traci.gui.setZoom(viewID='View #0', zoom=500)
        except Exception as exc:
            print(f"GUI track setup failed: {exc}")
        ...
        if self.sumo_control_state:
            ...
            if self.gui_flag:
                if self.gui_start:
                    sumoCmd += ["--start"]
                if self.gui_delay > 0:
                    sumoCmd += ["--delay", str(self.gui_delay)]
                print("SUMO GUI command:", " ".join(f'"{item}"' if " " in item else item for item in sumoCmd))
            if libsumo_flag:
                traci.start(sumoCmd)
            else:
    ...
    def set_zoom(self, zoom):
        ...
        try:
            ...
            print(f"GUI zoom setup failed: {exc}")
